...
const FREE_LIMIT = 3;
const REPO = "https://raw.githubusercontent.com/heidif68/Askacryptid/main/public/images";
    ...
    <div style={{ minHeight: "100vh", background: "#080808", fontFamily: "'Georgia', serif", color: "#e8e0d0", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 780, margin: "0 auto", padding: "2rem 1.2rem 5rem" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <p style={{ color: "#555", fontSize: "0.85rem", marginTop: "0.5rem", fontStyle: "italic" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.6rem" }}>
            ...
            {!isPro && (
              <div style={{ fontSize: "0.54rem", letterSpacing: "0.1em", textTransform: "uppercase", background: "#120e00", border: "1px solid #3a2e00", borderRadius: 2, padding: "0.12rem 0.45rem", color: "#776600" }}>
                Unlock for $4/mo
              </div>
            )}
            {isPro && (
        ...
        <div style={{ border: `1px solid ${isLocked ? "#181818" : c.accent + "2a"}`, borderRadius: 8, overflow: "hidden", marginBottom: "1rem", background: "#0c0c0c", transition: "border-color 0.5s" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "1.2rem 1.4rem", background: `linear-gradient(135deg, ${c.color}44 0%, #0c0c0c 100%)`, borderBottom: `1px solid ${c.accent}18`, transition: "all 0.5s" }}>
            <img
              src={c.image}
              alt={c.name}
              style={{ width: 72, height: 72, borderRadius: 8, objectFit: "cover", border: `1px solid ${c.accent}33`, filter: isLocked ? "grayscale(1) brightness(0.4)" : "none", transition: "filter 0.5s", flexShrink: 0 }}
            />
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <h2 style={{ margin: 0, fontSize: "1.2rem", fontWeight: 400, color: isLocked ? "#333" : c.accent, transition: "color 0.5s" }}>{c.name}</h2>
                {isLocked && <span style={{ fontSize: "0.58rem", color: "#443300", textTransform: "uppercase", border: "1px solid #2a1a00", padding: "0.1rem 0.35rem", borderRadius: 2 }}>Locked</span>}
              </div>
              <div style={{ color: "#444", fontSize: "0.7rem", fontStyle: "italic" }}>aka {c.aka}</div>
            </div>
          </div>

          {/* Locked state */}
          {isLocked && (
            <div style={{ textAlign: "center", padding: "2rem 1.4rem", color: "#444", fontStyle: "italic" }}>
            ...
            <div style={{ padding: "1.2rem 1.4rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
                  {sampleQuestions[c.id].map(q => (
                    <button key={q}
                      ...
                      style={{ background: "transparent", border: "1px solid #1c1c1c", borderRadius: 4, padding: "0.28rem 0.6rem", color: "#555", fontSize: "0.7rem", cursor: hitLimit ? "not-allowed" : "pointer", fontFamily: "Georgia, serif", fontStyle: "italic", transition: "all 0.2s" }}
                      onMouseEnter={e => { if (!hitLimit) { e.target.style.borderColor = c.accent + "55"; e.target.style.color = "#999"; }}}
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <input
                  ...
                  disabled={hitLimit}
                  style={{ flex: 1, background: "#111", border: "1px solid #1c1c1c", borderRadius: 4, padding: "0.7rem 1rem", color: hitLimit ? "#333" : "#e8e0d0", fontSize: "0.9rem", fontFamily: "Georgia, serif", outline: "none" }}
                />
                <button
                  onClick={() => hitLimit ? setShowUpgrade(true) : askCryptid()}
                  disabled={loading || (!hitLimit && !question.trim())}
                  style={{ background: hitLimit ? "#130f00" : (loading ? "#111" : c.accent + "22"), border: `1px solid ${hitLimit ? "#3a2800" : (loading ? "#1a1a1a" : c.accent + "55")}`, borderRadius: 4, padding: "0.7rem 1.2rem", color: hitLimit ? "#775500" : (loading ? "#2a2a2a" : c.accent), fontSize: "0.85rem", cursor: loading ? "not-allowed" : "pointer", fontFamily: "Georgia, serif", whiteSpace: "nowrap", transition: "all 0.3s" }}
                >
        ...
        {(loading || answer || error) && !isLocked && (
          <div ref={answerRef} style={{ border: `1px solid ${c.accent}18`, borderRadius: 8, padding: "1.5rem", background: "#070707", animation: "fadeIn 0.4s ease", marginBottom: "1rem" }}>
            ...
            {answer && !loading && (
              <p style={{ margin: 0, fontSize: "1rem", lineHeight: 1.85, color: "#c8c0b0", fontStyle: "italic" }}>
                "{answer}"
              </p>
            )}
            {error && <p style={{ margin: 0, color: "#444", fontStyle: "italic" }}>{error}</p>}
          </div>
        )}
...
function CryptidCard({ cr, selected, setSelected, setAnswer, locked }) {
  ...
  return (
    <button
      ...
      style={{
        background: isSelected ? (locked ? "#111" : cr.color + "88") : "#0e0e0e",
        border: `1px solid ${isSelected ? (locked ? "#333" : cr.accent + "66") : "#1a1a1a"}`,
